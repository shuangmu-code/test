<template>
  <div class="all">
    <div style="position:absolute;right:80px;z-index: 1000;padding:5px;border-radius:4px;background-color:rgb(255,255,255)" v-bind:class="{'dome1':use}">
    ...
    <el-card v-bind:class="{'dome1':active}" >
      ...
      <div class="mod-invoice-form">
        <el-form :inline="true"   size="mini" @keyup.enter.native="getDataList()">
          <el-row>
            <el-col :span="6" >
              <el-form-item label="发票号码：" >
                <el-input class="query-input" v-model="invoiceNumber" placeholder="请输入"></el-input>
              </el-form-item> 
            </el-col>
            <el-col :span="6" >
              <el-form-item label="交易流水号：">
                <el-input class="query-input" v-model="businessNumber" placeholder="请输入"></el-input>
              </el-form-item >
            </el-col>
            <el-col :span="6" >
              <el-form-item label="发票类型：">
                <el-radio-group  class="query-input" v-model="invoiceType" >
            ...
            <el-col :span="6" >
              <el-form-item label="购买方：" >
                <el-radio-group  class="query-input" v-model="buyer" >
                  <el-radio-button label="" >全部</el-radio-button>
          ...
          <el-row>
            <el-col :span="6" >
              <el-form-item label="业务类型：">  
                <el-select  class="query-input" v-model="businessType" placeholder="请选择"  >
                  <!-- 此处未绑定业务类型，待修改-->
            ...
            <el-col :span="6"  > 
              <el-form-item label="开票日期：">
                <el-date-picker v-model="date" class="query-input" type="daterange" range-separator="~" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
              </el-form-item> 
            </el-col>
            <el-col :span="6">
              <el-form-item label="发票状态：">
                <el-radio-group  class="query-input" v-model="status" >
                  <el-radio-button label="" >全部</el-radio-button>
                  <el-radio-button label="0">已开票</el-radio-button>
    ...
    <div class="area">
          ...
          <span class="img1" v-else ></span>
					...
					<span class="type" style="color:#FF7364;" v-show="item.kplx=='1'">红票</span>
          <span class="type" style="color:rgba(62,134,247,1);"  v-show="item.kplx=='0'">蓝票</span>
					<div class="table">
						<el-row >
							...
							<el-col :span='8' class="column1">发票状态</el-col>
              <el-col :span='16' class='column2' style="color:#52C41A" v-if="item.fpzt=='0'">已开票</el-col>
              <el-col :span='16' class='column2' style="color:#F5222D" v-if="item.fpzt=='1'">已作废</el-col>
          	</el-row>
					</div>
				  <el-row class="button">
						<el-button  class="button1"  round type="danger" v-show="item.kplx=='0'&&item.fpzt=='0'" @click="redHandle(index)">冲红</el-button>
						<el-button  class="button2"  round @click="toDetails(item.id)" v-show="item.kplx=='0'&&item.fpzt=='0'">发票详情</el-button>
            <el-button  class="button2" style="margin-left:65%" round @click="toDetails(item.id)" v-show="item.kplx=='1'||item.fpzt=='1'">发票详情</el-button>
					</el-row>
				 </div>
			</div>
  ...
  export default {
    ...
    methods: {
      getDataList(){
        ...
        var t_start = this.date[0] != undefined ? this.date[0].toLocaleDateString() : undefined
        var t_end = this.date[1] != undefined ? this.date[1].toLocaleDateString() : undefined
        this.$http({
            url: this.$http.adornUrl('/invoice/record/list'),
            method: 'get',
            params: this.$http.adornParams({
                'page':this.currPage,
                'limit':8,
                'fphm':this.invoiceNumber,
                'ywlx':this.businessType,
                'fplsh':this.businessNumber,
                'timeStart':t_start,
                'timeEnd':t_end,
                'kplx':this.invoiceType,
                'fpzt':this.status,
                'gmflx':this.buyer
            })
      }).then(({data}) => {
      ...
      redHandle(index) {
        this.$confirm('确认要将该发票冲红吗?', '温馨提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$http({
              url: this.$http.adornUrl(`/invoice/record/red/${this.datalist[index].id}`),
              method: 'post',
              params: this.$http.adornParams()
          }).then(({data}) => {
              if (data && data.code === 0) {
